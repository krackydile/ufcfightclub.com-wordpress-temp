...
export class PhotonicPhotoSwipe extends Lightbox {
	...
	hostedVideo(a) {
		...
		let css = a.classList.contains('photonic-launch-gallery');
...
//			if (parent != null) {
				...
				const links = container.querySelectorAll('.photonic-launch-gallery');
				const gallery = [];
...
//			}
		...
		const a = document.querySelectorAll('a.launch-gallery-photoswipe');
	...
	initializeForExisting() {
		...
		document.addEventListener('click', e => {
			...
			e.preventDefault();
			const clicked = e.target.closest('a.launch-gallery-photoswipe'); //e.currentTarget;
