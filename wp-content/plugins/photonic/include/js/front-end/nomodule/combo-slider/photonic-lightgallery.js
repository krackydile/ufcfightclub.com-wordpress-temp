  ...
  (0, _createClass2.default)(PhotonicLightGallery, [{
    ...
    value: function initialize(selector, selfSelect) {
      ...
      selection.forEach(function (current) {
        ...
        var thumbs;
        thumbs = current.querySelectorAll('a.launch-gallery-lightgallery');
        var rel = '';
        ...
        current.addEventListener('onAfterSlide', function (event) {
          thumbs = current.querySelectorAll('a.launch-gallery-lightgallery'); // Need to fetch again, since the next line causes issues after "Add More".

          var thumb;

          if (thumbs.length !== 0) {
            thumb = thumbs[event.detail.index];
          } else if (current.classList.contains('launch-gallery-lightgallery')) {
            thumb = current;
          }
